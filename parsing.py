    ...
    def compileClass(words):
        ...
        def classBody(words):
            if len(words) == 0:
                ...
                return [Parser.compileClassVarDec(words[0:i+1])] + classBody(words[i+1:])
            elif ismethod(words[0][1]):
                i = Parser.nextblock(words, ismethod) # small function that finds where current method ends
                return [Parser.compileSubDec(words[0:i])] + classBody(words[i:])
            else:
        ...
        return ans + classBody(words[3:-1]) + [words[-1]]
    
    
    #done
    def compileClassVarDec(words):
        return ['classVarDec'] + words
    
    
    #done
    def compileSubDec(words):
        i = words.index(['symbol', ')'])
        
        ans = ['subroutineDec'] + words[0:4]
        ans += [Parser.compileParamList(words[4:i])]
        ans += [['symbol', ')']]
        ans += [Parser.compileSubBody(words[i+1:])]
        return  ans
    
    
    #done
    def compileParamList(words):
        return ["parameterList"] + words
    
    
    #done
    def compileSubBody(words):
...
EX1 = Parser.compileClass(tokenizer("""
class program{
...
EX2 = Parser.compileClass(tokenizer("""
class program{
    field a;
    field b;
    
    constructor void program(){}
    method int foo(a, b, c){}  
}
"""
))

EX3 = Parser.compileSubDec(tokenizer("""
method int foo(a, b, c){
    return 0;
}  
"""
